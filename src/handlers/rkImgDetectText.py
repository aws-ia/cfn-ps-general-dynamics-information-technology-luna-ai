...
def LambdaHandler(event, context):
    ...
    sEnrichments = generateEnrichments(response, "aws-recognizetext", event["detail"]["AssetId"], sS3RkTxtDet, sKey)
    ...
    if 'Enrichments' in sEnrichments:
        persistDetectiontoS3(event, response["TextDetections"], event["detail"]["AssetId"], sS3RkTxtDet, sKey)
        enrichmentEvent(event, sEnrichments, "rekognition", "", "detecttext", sS3RkTxtDet, sKey, sSrcExtension)
...
def detect(event, sBucket, sKey, sExt):
    response = clientRk.detect_text(
        ...
        Filters={
            'WordFilter': {
                'MinConfidence': float(srkConfidence)
            }
        }
    )
    return response
...
def generateEnrichments(response, sProcessType, sAssetId, sBucket, sKey):
    sAggregate = ""
    enrichments = []
    if response.get('TextDetections'):
        for item in response["TextDetections"]:
            if item["Type"] == "LINE":
                enrichmentItem = {"Name": item["DetectedText"], "ParentName": "", "Confidence": item["Confidence"]}
                enrichments.append(enrichmentItem)

                writeSearchEnrichmentDynamoDB(sProcessType, sAssetId, item["DetectedText"], float(item["Confidence"]))
                sAggregate = sAggregate + item["DetectedText"] + ", "

        data = {}
        data["Enrichments"] = enrichments
        writeSearchEnrichmentAggregateDynamoDB(sProcessType, sAssetId, sAggregate, response["TextDetections"], sBucket, sKey)
